...
namespace Utf8Json.Resolvers
{
    ...
    public sealed class DictionaryKeyCamelCaseDynamicGenericResolver : IJsonFormatterResolver
    {
        public static readonly IJsonFormatterResolver Instance = new DictionaryKeyCamelCaseDynamicGenericResolver();

        DictionaryKeyCamelCaseDynamicGenericResolver()
        {
        }
        ...
        static class FormatterCache<T>
        {
            ...
            static FormatterCache()
            {
                formatter = (IJsonFormatter<T>)DynamicGenericResolverGetFormatterHelper.GetDictionaryKeyCamelCaseFormatter(typeof(T));
            }
        }
    }
}
...
namespace Utf8Json.Resolvers.Internal
{
    internal static class DynamicGenericResolverGetFormatterHelper
    {
        ...
        static readonly Dictionary<Type, Type> camelCaseFormatterMap = new Dictionary<Type, Type>()
        {
            {typeof(List<>), typeof(ListFormatter<>)},
            {typeof(LinkedList<>), typeof(LinkedListFormatter<>)},
            {typeof(Queue<>), typeof(QeueueFormatter<>)},
            ...
            {typeof(ICollection<>), typeof(InterfaceCollectionFormatter<>)},
            {typeof(IEnumerable<>), typeof(InterfaceEnumerableFormatter<>)},
            {typeof(Dictionary<,>), typeof(CamelCaseDictionaryFormatter<,>)},
            {typeof(IDictionary<,>), typeof(CamelCaseInterfaceDictionaryFormatter<,>)},
            {typeof(SortedDictionary<,>), typeof(CamelCaseSortedDictionaryFormatter<,>)},
            {typeof(SortedList<,>), typeof(SortedListFormatter<,>)},
            {typeof(ILookup<,>), typeof(InterfaceLookupFormatter<,>)},
            {typeof(IGrouping<,>), typeof(InterfaceGroupingFormatter<,>)},
#if NETSTANDARD
            ...
            {typeof(System.Collections.Concurrent.ConcurrentQueue<>), typeof(ConcurrentQueueFormatter<>)},
            {typeof(System.Collections.Concurrent.ConcurrentStack<>), typeof(ConcurrentStackFormatter<>)},
            {typeof(ReadOnlyDictionary<,>), typeof(CamelCaseReadOnlyDictionaryFormatter<,>)},
            {typeof(IReadOnlyDictionary<,>), typeof(CamelCaseInterfaceReadOnlyDictionaryFormatter<,>)},
            {typeof(System.Collections.Concurrent.ConcurrentDictionary<,>), typeof(CamelCaseConcurrentDictionaryFormatter<,>)},
            {typeof(Lazy<>), typeof(LazyFormatter<>)},
            {typeof(Task<>), typeof(TaskValueFormatter<>)},
#endif
        };
        ...
        internal static object GetFormatter(Type t)
        {
            ...
            else if (ti.IsGenericType)
            {
                ...
                else if (genericType == typeof(ValueTask<>))
                {
                    return CreateInstance(typeof(ValueTaskFormatter<>), ti.GenericTypeArguments);
                         ...
                         nullableElementType.GetGenericTypeDefinition() == typeof(ValueTask<>))
                {
                    return CreateInstance(typeof(NullableFormatter<>), new[] {nullableElementType});
                }
            ...
            else
            {
                ...
                else if (typeof(IDictionary).GetTypeInfo().IsAssignableFrom(ti) &&
                         ti.DeclaredConstructors.Any(x => x.GetParameters().Length == 0))
                {
        ...
        internal static object GetDictionaryKeyCamelCaseFormatter(Type t)
        {
            var ti = t.GetTypeInfo();
            ...
            else if (ti.IsGenericType)
            {
                ...
                {
                    Type formatterType;
                    if (camelCaseFormatterMap.TryGetValue(genericType, out formatterType))
                    {
                        return CreateInstance(formatterType, ti.GenericTypeArguments);
                    }
                             ...
                             && ti.DeclaredConstructors.Any(x => x.GetParameters().Length == 0))
                    {
                        var keyType = ti.GenericTypeArguments[0];
                        var valueType = ti.GenericTypeArguments[1];
                        return CreateInstance(typeof(CamelCaseGenericDictionaryFormatter<,,>), new[] {keyType, valueType, t});
                    }
                }
            }
            else
